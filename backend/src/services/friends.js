...
export async function sendFriendRequest(requesterId, addresseeUsername) {
  ...
  return await friendQueries.createFriendRequest(requesterId, addressee.id);
}
...
export async function acceptRequest(friendshipId, userId) {
  ...
  return await friendQueries.acceptFriendRequest(friendshipId, userId);
}
...
export async function getDMHistory(userId, otherId, beforeId = null, limit = 50) {
  ...
  const result = await require('../db/index.js').default.query(query, params);
